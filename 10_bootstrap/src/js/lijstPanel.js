...
export async function searchConsoles(event) {
    ...
    console.log(typeof searchFieldValue)
    try {
...
export async function handleConsoleTable(ev) {
    if (ev.target.classList.contains("deleteButton")) {
        ...
        document.querySelector("#lijst").classList.remove("active")
...
export async function fillConsolesTable(searchFieldValue) {
    ...
    consolesTable.innerHTML = ''
    if (consoles.length !== 0) {
        consolesTable.innerHTML += '<thead class="console-id text-white bg-primary fw-bold"><tr><th>ID</th><th>Console</th><th>Generation</th></tr></thead>'
    }

    let tbody = document.createElement("tbody")
    consoles.forEach(c => tbody.append(createRow(c)))
    consolesTable.append(tbody)
